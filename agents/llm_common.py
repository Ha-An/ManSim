...
class OptionalLLMDecisionModule(DecisionModule):
    ...
    def __init__(self, cfg: dict[str, Any], llm_cfg: dict[str, Any] | None = None) -> None:
        ...
        raw_llm_seed = self.llm_cfg.get("seed", cfg.get("seed", None))
        self.llm_inference_seed = None if raw_llm_seed in {None, ""} else int(raw_llm_seed)
        ...
        if self.provider == "openclaw":
            allowed_openclaw_transports = {"native_local", "chat_compat", "backend_direct"}
            if self.openclaw_transport not in allowed_openclaw_transports:
                self._fail("Unsupported decision.llm.openclaw.transport. Allowed values: native_local, chat_compat, backend_direct.")
            if self.openclaw_phase_transport_default not in allowed_openclaw_transports:
                self._fail("Unsupported decision.llm.openclaw.phase_transport.default. Allowed values: native_local, chat_compat, backend_direct.")
        ...
        if self.provider == "openclaw":
            self.openclaw_client = OpenClawClient(
                ...
                inference_seed=self.llm_inference_seed,
            )
    ...
    def _openclaw_transport_for_call(self, call_name: str) -> str:
        if not self._openclaw_enabled():
            return "chat_compat"
        normalized = str(call_name or "").strip()
        if normalized in self.openclaw_chat_phase_names:
            return "chat_compat"
        if normalized in self.openclaw_native_phase_names:
            return "native_local"
        selected = str(self.openclaw_phase_transport_default or self.openclaw_transport or "native_local").strip().lower()
        return selected if selected in {"native_local", "chat_compat", "backend_direct"} else "native_local"

    def _openclaw_uses_transport(self, transport: str) -> bool:
        if not self._openclaw_enabled():
            return False
        mode = str(transport or "").strip().lower()
        return mode in {"native_local", "chat_compat", "backend_direct"}

    def _openclaw_backend_only_health(self, *, force: bool = False) -> dict[str, Any]:
        if not self._openclaw_enabled() or self.openclaw_client is None:
            return {"ok": True, "checked_at": time.time(), "backend": {"ok": True}, "reason": None}
        provider = str(self.openclaw_backend.get("provider", "")).strip().lower()
        base_backend = self.openclaw_client._effective_backend_base_url().rstrip("/")
        probe_path = self.openclaw_client._backend_probe_path(provider)
        ...
        if probe_path:
            ...
            if base_backend:
                endpoint = self.openclaw_client._join_api_probe_url(base_backend, probe_path)
                probe = self.openclaw_client._probe_http_endpoint(endpoint, timeout_sec=float(self.openclaw_backend_health_probe_timeout_sec or 2.5))
        ...
        return {
            "ok": bool(backend_probe.get("ok", False)),
            "checked_at": time.time(),
            "backend": backend_probe,
            "reason": None if backend_probe.get("ok", False) else "backend_health_check_failed",
        }

    def _backend_direct_completion(
        self,
        *,
        ...
        base_url = str(self.openclaw_backend.get("base_url", "")).strip().rstrip("/")
        if not base_url:
            raise RuntimeError("OpenClaw backend_direct transport requires backend.base_url.")
        url = base_url + "/chat/completions"
        backend_model = str(self.openclaw_backend.get("model", "")).strip()
        if not backend_model:
            raise RuntimeError("OpenClaw backend_direct transport requires backend.model.")
        backend_limit = max(64, int(self.openclaw_backend.get("max_output_tokens", max_tokens) or max_tokens))
        effective_max_tokens = max(32, min(int(max_tokens), backend_limit))
        headers_for_log = {"Content-Type": "application/json"}
        req_headers = {"Content-Type": "application/json"}
        api_key = str(self.openclaw_backend.get("api_key", self.api_key)).strip()
        if api_key:
        ...
        if self.llm_inference_seed is not None:
            request_payload["seed"] = int(self.llm_inference_seed)
        ...
        try:
            ...
            raise RuntimeError(f"OpenClaw backend_direct response format error: {exc}") from exc
        return payload, content, url, headers_for_log, request_payload
    ...
    def _openclaw_chat_readycheck(self, *, agent_id: str, call_name: str, context: dict[str, Any] | None = None) -> dict[str, Any]:
        if not self._openclaw_enabled() or self.openclaw_client is None:
            return {"ok": True, "reason": "openclaw_disabled"}
        ready_context = dict(context or {})
        ready_context["healthcheck"] = True
        session_key = self._openclaw_turn_session_key(agent_id, f"{call_name}_chat_readycheck", ready_context)
        started = time.time()
        original_timeout = int(getattr(self.openclaw_client, "timeout_sec", self.timeout_sec) or self.timeout_sec)
        try:
            self.openclaw_client.timeout_sec = max(1, int(self.openclaw_runtime_readiness_timeout_sec or original_timeout))
            payload, content, _url, _headers, _body = self.openclaw_client.chat_completion(
                messages=[
                    {
                        "role": "system",
                        "content": (
                            "Readiness check. Return exactly one JSON object with the field status set to ready. "
                            "Do not add prose, markdown, or extra keys."
                        ),
                    },
                    {"role": "user", "content": 'Return exactly {"status":"ready"}.'},
                ],
                temperature=0.0,
                max_tokens=min(64, max(32, int(self.max_tokens or 64))),
                agent_id=agent_id,
                session_key=session_key,
            )
            parsed = self._extract_json_object(content)
    ...
    def _openclaw_recover_runtime(self, *, call_name: str, context: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        call_transport = self._openclaw_transport_for_call(call_name)
        ...
        def _probe_then_ready(stage: str) -> tuple[dict[str, Any], dict[str, Any]]:
            ...
            if call_transport == "chat_compat":
                ready = self._openclaw_chat_readycheck(agent_id=agent_id, call_name=call_name, context=context)
            else:
    ...
    def _openclaw_write_markdown(self, path: Path, title: str, sections: list[tuple[str, Any]]) -> None:
        text = self._openclaw_render_markdown(title, sections)
        last_error: Exception | None = None
        for attempt in range(4):
            try:
                path.parent.mkdir(parents=True, exist_ok=True)
    ...
    def _seed_openclaw_run_context(self) -> None:
        ...
        runtime_agent_ids = list(dict.fromkeys([
            str(agent_id).strip()
            for agent_id in list((self.openclaw_runtime_workspace_aliases or {}).keys())
            if str(agent_id).strip()
        ]))
        if not runtime_agent_ids:
            runtime_agent_ids = list(dict.fromkeys(list(self.openclaw_worker_agent_ids) + [self.openclaw_manager_agent_id]))
        visible_runtime_agent_ids = list(
            dict.fromkeys(
                [
                    str((self.openclaw_runtime_workspace_aliases or {}).get(str(agent_id).strip().upper(), str(agent_id).strip().upper())).strip().upper()
                    for agent_id in runtime_agent_ids
                    if str(agent_id).strip()
                ]
            )
        )
        run_context_internal = {
        ...
        run_context_visible = {
            "language": run_context_internal["language"],
            "objective": run_context_internal["objective"],
            "total_days": run_context_internal["total_days"],
            "minutes_per_day": run_context_internal["minutes_per_day"],
            "worker_agent_ids": list(run_context_internal["worker_agent_ids"]),
            "manager_agent_id": run_context_internal["manager_agent_id"],
            "runtime_agent_ids": visible_runtime_agent_ids,
            "run_isolation": True,
        }
        runtime_facts_root = getattr(self, "openclaw_runtime_facts_root", None)
        if runtime_facts_root is not None:
            self._openclaw_write_json(Path(runtime_facts_root) / "run_context_internal.json", dict(run_context_internal))
        for agent_id in runtime_agent_ids:
            workspace = self._openclaw_workspace_path(agent_id)
            if workspace is None:
                continue
            visible_agent_id = str((self.openclaw_runtime_workspace_aliases or {}).get(str(agent_id).strip().upper(), str(agent_id).strip().upper())).strip().upper()
            self._openclaw_write_json(workspace / "facts" / "run_context.json", {**run_context_visible, "agent_id": visible_agent_id})
            self._openclaw_write_markdown(
                workspace / "RUN_CONTEXT.md",
                f"{visible_agent_id} Run Context",
                [
                    ("Scope", "This workspace is isolated to the current simulation run only. Do not assume any prior-run continuity."),
                    ("Objective", run_context_visible["objective"]),
                    ("Run Metadata", {**run_context_visible, "agent_id": visible_agent_id}),
                ],
            )
            self._openclaw_write_markdown(
                workspace / "memory" / "rolling_summary.md",
                f"{visible_agent_id} Rolling Summary",
                [
                    ("Run Scope", "This file only accumulates memory for the current simulation run."),
                    ("Latest Summary", {"status": "initialized", "run_isolation": True}),
                ],
            )
            self._openclaw_write_json(workspace / "beliefs" / "current_beliefs.json", {"status": "initialized", "agent_id": visible_agent_id, "run_isolation": True})
            self._openclaw_write_json(workspace / "commitments" / "current_commitment.json", {"status": "initialized", "agent_id": visible_agent_id, "run_isolation": True})
            self._openclaw_write_json(workspace / "memory" / "semantic" / "current.json", {"status": "initialized", "agent_id": visible_agent_id, "run_isolation": True})
    ...
    def _close_unbalanced_json_like_text(self, text: str) -> str:
    ...
    def _repair_json_like_text(self, text: str) -> str:
        ...
        repaired = self._close_unbalanced_json_like_text(repaired)
    ...
    def _call_llm_json(
        ...
        *,
        ...
        if self._openclaw_enabled() and current_transport not in {"native_local", "chat_compat", "backend_direct"}:
            raise RuntimeError("Unsupported OpenClaw transport requested during call.")
        ...
        def _native_contract_repair_prompt(contract_issues: list[str] | None = None) -> str:
            ...
            if phase_name == "manager_daily_planner":
                return (
                    ...
                    + "). Re-read facts/current_request.json and facts/current_response_template.json in your workspace. "
                    + "Return exactly one JSON object only. "
                    + "For manager_daily_planner, emit only plan_mode, commitments, mailbox, incident_strategy, reason_trace, and detector_alignment. "
                    + "commitments must be a dict keyed by A1, A2, and A3, and each commitment must reference a valid opportunities.opportunity_id. "
                    + "mailbox must be a dict keyed by A1, A2, and A3. "
                    + "incident_strategy must be an object with mode, focus_opportunity_ids, and watchouts. "
                    + "detector_alignment must be follow, partial_override, or override. "
                    + "Use execution_state, closure_signals, constraint_signals, detector_hypothesis, opportunities, and incident_context as evidence. "
                    ...
                    + "Do not output queue_add, weight_updates, mailbox_add, agent_multiplier_updates, maintain_current_plan, stability_reason, or generic prose reasoning. "
                    + "Use the correct empty type only."
                )
            if phase_name == "manager_shift_strategist":
                return (
                    "Previous reply violated the shift strategist JSON contract ("
                    + issue_text
                    + "). Return exactly one compact JSON object only. "
                    + "Emit only summary, worker_roles, operating_focus, late_horizon_mode, role_plan, support_plan, prevention_targets, daily_targets, and plan_revision. "
                    + "worker_roles values must be short canonical role strings. "
                    + "support_plan must contain primary_support_pair and support_intent. "
                    + "prevention_targets must use only canonical prevention labels from the request packet. "
                    + "daily_targets must be a compact object with only the supported target keys. "
                    + "Do not emit task_priority_weights, agent_priority_multipliers, mailbox_seed, or commitments. "
                    + "If the prior reply was cut off, make this reply shorter."
                )
            if phase_name == "manager_daily_reviewer":
                return (
                    "Previous reply violated the daily reviewer JSON contract ("
                    + issue_text
                    + "). Return exactly one compact JSON object only. "
                    + "Emit only target_misses, top_failure_modes, recommended_prevention_targets, recommended_support_pair, role_change_advice, and carry_forward_risks. "
                    + "Do not restate raw metrics from the request packet. "
                    + "Use only canonical failure/prevention labels. "
                    + "recommended_support_pair must be exactly one pair such as A1->A3. "
                    + "Do not emit task_priority_weights, mailbox, norms, prose summary blocks, or commitments. "
                    + "If the prior reply was cut off, make this reply shorter."
                )
            return (
        ...
        def _native_structural_failure_reason(raw_content: str, raw_payload: dict[str, Any] | None) -> str:
            text = str(raw_content or "")
            payload_meta = raw_payload.get("meta") if isinstance(raw_payload, dict) else {}
            stop_reason = ""
            if isinstance(payload_meta, dict):
                stop_reason = str(payload_meta.get("stopReason", "") or "").strip().lower()
            if "Context overflow: prompt too large for the model." in text:
                return "context_overflow"
            if "Compaction safeguard: no real conversation messages to summarize" in text:
                return "compaction_safeguard_no_messages"
            if stop_reason == "length" and isinstance(raw_payload, dict):
                payloads = raw_payload.get("payloads")
                if isinstance(payloads, list) and not payloads:
                    return "empty_payload_stop_reason_length"
            return ""
        ...
        try:
            while attempts < 3:
                ...
                if self._openclaw_enabled():
                    if current_transport not in {"native_local", "chat_compat", "backend_direct"}:
                        return _return_default(
                            "Unsupported OpenClaw transport requested during call.",
                            fields=list(expected_contract.keys()),
                        )
                    if current_transport == "backend_direct":
                        latest_backend_health = self._openclaw_backend_only_health()
                    else:
                    ...
                    if not latest_backend_health.get("ok", False):
                        ...
                        if current_transport != "backend_direct":
                            ...
                            attempts += 1
                            time.sleep(max(0.25, float(self.openclaw_runtime_recovery_backoff_sec or 0.25)))
                            continue
                    ...
                    elif current_transport == "chat_compat":
                        try:
                            payload, content, url, headers_for_log, body = self.openclaw_client.chat_completion(
                                messages=messages,
                            ...
                            if attempts < 2:
                                ...
                                continue
                            return _return_default(
                                f"OpenClaw chat_compat call failed before satisfying JSON contract: {type(exc).__name__}: {exc}",
                                fields=list(expected_contract.keys()),
                            )
                    else:
                        try:
                            payload, content, url, headers_for_log, body = self._backend_direct_completion(
                                messages=messages,
                                temperature=self.temperature,
                                max_tokens=self.max_tokens,
                            )
                        except Exception as exc:
                            attempt_errors.append(f"backend_direct_attempt:{type(exc).__name__}:{exc}")
                            attempt_latencies_ms.append(round((time.time() - attempt_start) * 1000.0, 3))
                            if attempts < 2:
                                attempts += 1
                                messages = messages + [
                                ...
                                continue
                            return _return_default(
                                f"OpenClaw backend_direct call failed before satisfying JSON contract: {type(exc).__name__}: {exc}",
                                fields=list(expected_contract.keys()),
                            )
                else:
                    headers_for_log = {"Content-Type": "application/json"}
                    ...
                    if self.llm_inference_seed is not None:
                        body["seed"] = int(self.llm_inference_seed)
                ...
                attempt_latencies_ms.append(round((time.time() - attempt_start) * 1000.0, 3))
                parsed = self._extract_json_object(content)
                structural_failure = _native_structural_failure_reason(content, payload if isinstance(payload, dict) else {})
                ...
                if isinstance(parsed, dict):
                    if expected_contract:
                        ...
                        if contract_issues:
                            if self._openclaw_enabled() and structural_failure:
                                return _return_default(
                                    f"Native JSON contract violation after non-repairable native response ({structural_failure}): {' ,'.join(contract_issues)[:240]}",
                            ...
                            if attempts < 2:
                                ...
                                if self._openclaw_enabled() and current_transport == "native_local":
                                    continue
                                repair_text = native_repair_prompt or repair_instruction
                                messages = messages + [
                                    {"role": "assistant", "content": content},
                ...
                if self._openclaw_enabled() and structural_failure:
                    return _return_default(
                        f"Native response is non-repairable ({structural_failure}); skipping retry.",
                        fields=list(expected_contract.keys()),
                    )
                ...
                if self._openclaw_enabled() and current_transport == "native_local":
                    ...
                    repair_text = _native_contract_repair_prompt(["invalid_json"]) if self._openclaw_enabled() else repair_instruction
                    messages = messages + [
                        {"role": "assistant", "content": content},
                        {"role": "user", "content": repair_text},
                    ]
