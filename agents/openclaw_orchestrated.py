...
class OpenClawOrchestratedDecisionModule(OptionalLLMDecisionModule):
    ...
    def __init__(self, cfg: dict[str, Any], llm_cfg: dict[str, Any] | None = None) -> None:
        ...
        incident_cfg = orch_cfg.get("incident", {}) if isinstance(orch_cfg.get("incident", {}), dict) else {}
        self.incident_replan_enabled = bool(incident_cfg.get("enabled", True))
        ...
        self.last_diagnosis_payload: dict[str, Any] = {}
    ...
    def _compact_plan_focus(self, weights: Any, queues: Any, commitments: Any = None) -> dict[str, Any]:
        ...
        commitment_map = commitments if isinstance(commitments, dict) else {}
        commitment_focus: dict[str, list[str]] = {}
        for agent_id in self.agent_ids:
            rows = commitment_map.get(agent_id, []) if isinstance(commitment_map.get(agent_id, []), list) else []
            ...
            for item in rows[:2]:
                ...
                token = f"{task}#{opportunity_id}" if task and opportunity_id else (task or opportunity_id)
                if token:
                    compact_rows.append(token)
            if compact_rows:
                commitment_focus[agent_id] = compact_rows
        return {
            "top_weighted_tasks": [
    ...
    def _planner_prompt_memory_payload(self, latest_entry: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
        recent_plans = [
            {
                "day": int(item.get("day", 0) or 0),
                "summary": str(item.get("summary", "")).strip(),
                **self._compact_plan_focus(
                    item.get("task_priority_weights", {}),
                    item.get("personal_queues", {}),
        ...
        if empty_queue_days >= 2 and not any(item.get("commitment_focus") for item in recent_plans):
            watchouts.append("repeated_sparse_commitment_focus")
        latest_focus = self._compact_plan_focus(
            latest_entry.get("task_priority_weights", {}),
            latest_entry.get("personal_queues", {}),
            latest_entry.get("commitments", {}),
        )
        payload = {
            "latest_plan_focus": {
        ...
        commitment = {
            ...
            or [
                item.split("#", 1)[0]
                for rows in latest_focus.get("commitment_focus", {}).values()
                for item in rows[:2]
                if item
            ],
        ...
        *,
        ...
        if str(phase) == "manager_daily_planner":
            request_payload["reason_trace_contract"] = {
                "entry": {
                    ...
                    "reason": "str",
                    "evidence": [{"signal": "str", "value": "number|string|bool", "source": "execution_state|closure_signals|constraint_signals|detector_hypothesis|opportunities|incident_context|guardrails"}],
                    "affected_agents": ["A1|A2|A3"],
                    "task_families": ["allowed_task_priority_key"],
                    "detector_relation": "follow|reject|deprioritize",
                }
            }
            request_payload["commitment_contract"] = {
                "shape": {
                    "A1|A2|A3": [
                        {
                            "opportunity_id": "str_from_opportunities",
                            "commitment_id": "str",
                            "alternate_workers": ["A1|A2|A3"],
                            "dependencies": ["commitment_id"],
                            "expiry_min": "number|null",
                            "success_criteria": ["str"],
                            "rationale": "str",
                        }
                    ]
                },
                "rule": "Use opportunities.opportunity_id directly. Do not invent targets that are not present in the current opportunity board.",
            }
            request_payload["mailbox_contract"] = {
                "shape": {
                    "A1|A2|A3": [
                        {
                            "to_agent": "A1|A2|A3",
                            "message_type": "handover|coordination|dependency|watchout|assist_request",
                            "body": "str",
                        }
                    ]
                },
            ...
            request_payload["decision_contract"] = [
                ...
                "Use commitments as the authoritative execution output. Do not rely on generic weight changes or queue-like target guesses.",
                "Each commitment must reference a valid opportunities.opportunity_id and should respect ownership and capacity constraints implied by the opportunity board.",
                "Use maintain only when no materially stronger commitment set is justified by current evidence and relevant run-local memory.",
                "Set detector_alignment to follow, partial_override, or override.",
                "If opportunities is non-empty and plan_mode=adjust, commitments must contain at least one worker-specific executable commitment.",
            ]
        ...
        if str(phase) == "manager_daily_planner":
            user_prompt = (
                ...
                "Return exactly one JSON object with plan_mode, commitments, mailbox, incident_strategy, reason_trace, and detector_alignment."
            )
        if str(phase) == "manager_run_reflector":
            user_prompt = (
    ...
    def _current_opportunity_rows(self, observation: dict[str, Any]) -> list[dict[str, Any]]:
        rows = observation.get("opportunities", []) if isinstance(observation.get("opportunities", []), list) else []
        ...
        for item in rows:
            ...
            if not opportunity_id or task_family not in self.allowed_task_priority_keys:
                continue
            ...
            cleaned.append(
                {
                    ...
                    "capacity": max(1, int(item.get("capacity", 1) or 1)),
                    "owners": [aid for aid in self._as_str_list(item.get("owners"), []) if aid in self.agent_ids],
                    "why_available": self._truncate_prompt_text(item.get("why_available", ""), max_len=180),
                    "expected_output_impact": round(float(item.get("expected_output_impact", 0.0) or 0.0), 3),
    ...
    @staticmethod
    def _opportunity_target(opportunity: dict[str, Any]) -> dict[str, Any]:
    ...
    def _sanitize_incident_strategy(self, src: Any) -> dict[str, Any]:
        strategy = src if isinstance(src, dict) else {}
        return {
            "mode": self._truncate_prompt_text(strategy.get("mode", "delta_replan"), max_len=32) or "delta_replan",
            "focus_opportunity_ids": [str(item).strip() for item in self._as_str_list(strategy.get("focus_opportunity_ids"), []) if str(item).strip()][:6],
            "watchouts": [self._truncate_prompt_text(item, max_len=160) for item in self._as_str_list(strategy.get("watchouts"), []) if self._truncate_prompt_text(item, max_len=160)][:4],
        }

    def _compact_planner_incident_context(self, incident_context: Any) -> dict[str, Any]:
        blob = incident_context if isinstance(incident_context, dict) else {}
        recent_incidents = blob.get("recent_incidents", []) if isinstance(blob.get("recent_incidents", []), list) else []
        active_blockers = blob.get("active_blockers", []) if isinstance(blob.get("active_blockers", []), list) else []
            ...
            compact_incidents.append(
                {
                    ...
                    "incident_class": str(item.get("incident_class", "")).strip(),
                    "time_min": item.get("time_min"),
                    "affected_entities": [str(value) for value in list(item.get("affected_entities", []))[:4]],
                    "reason": self._truncate_prompt_text(details.get("reason", ""), max_len=96),
                    "blocker_id": str(details.get("blocker_id", "")).strip(),
                }
            )
        compact_blockers: list[dict[str, Any]] = []
        for item in active_blockers[:4]:
            if not isinstance(item, dict):
                continue
            compact_blockers.append(
                {
                    ...
                    "blocker_type": str(item.get("blocker_type", "")).strip(),
                    "candidate_signature_hash": str(item.get("candidate_signature_hash", "")).strip(),
                    "active_plan_revision": int(item.get("active_plan_revision", 0) or 0),
                    "escalation_emitted": bool(item.get("escalation_emitted", False)),
                }
            )
        return {
            "recent_incidents": compact_incidents,
            "active_blockers": compact_blockers,
        }

    def _sanitize_commitments(self, src: Any, observation: dict[str, Any], fallback: JobPlan | None = None) -> dict[str, list[dict[str, Any]]]:
        out = {agent_id: [] for agent_id in self.agent_ids}
        if not isinstance(src, dict):
            return out
        opportunities = self._current_opportunity_rows(observation)
        opportunity_index = {str(item.get("opportunity_id", "")).strip(): item for item in opportunities}
        assigned_counts: dict[str, int] = defaultdict(int)
        if isinstance(fallback, JobPlan) and isinstance(fallback.commitments, dict):
            for rows in fallback.commitments.values():
                if not isinstance(rows, list):
                    continue
                for item in rows:
                    if not isinstance(item, dict):
                        continue
                    opp_id = str(item.get("opportunity_id", "")).strip()
                    if opp_id:
                        assigned_counts[opp_id] += 1
        for agent_id in self.agent_ids:
            raw_rows = src.get(agent_id, [])
            if not isinstance(raw_rows, list):
                continue
            cleaned: list[dict[str, Any]] = []
            for idx, item in enumerate(raw_rows[: self.worker_queue_limit], start=1):
                if not isinstance(item, dict):
                    continue
                opportunity_id = str(item.get("opportunity_id", "")).strip()
                ...
                cleaned.append(
                    {
                        "commitment_id": str(item.get("commitment_id", f"CMT-{agent_id}-{idx}"))[:64],
                        "opportunity_id": opportunity_id,
                        "task_family": str(opportunity.get("task_family", "")).strip(),
                        "assigned_worker": agent_id,
                        "target": self._opportunity_target(opportunity),
                        "alternate_workers": [aid for aid in self._as_str_list(item.get("alternate_workers"), []) if aid in self.agent_ids and aid != agent_id][:2],
                        "dependencies": self._as_str_list(item.get("dependencies"), [])[:4],
                        "expiry_min": item.get("expiry_min"),
                        "handoff_policy": "allowed" if self.worker_local_response_cfg.get("allow_handoff", True) else "manager_only",
                        "success_criteria": [self._truncate_prompt_text(value, max_len=140) for value in self._as_str_list(item.get("success_criteria"), [])[:3]],
                        "rationale": self._truncate_prompt_text(item.get("rationale", opportunity.get("why_available", "")), max_len=220),
                        "source": "planner_commitment",
                        "status": "planned",
                        "metadata": {
                            "expected_output_impact": opportunity.get("expected_output_impact"),
    ...
    def _sanitize_mailbox(self, src: Any) -> dict[str, list[dict[str, Any]]]:
        ...
        if not isinstance(src, dict):
            return out
        valid_message_types = {"handover", "coordination", "watchout", "dependency", "assist_request", "focus_window"}
        valid_target_types = {"none", "station", "machine", "agent", "location"}
        for agent_id in self.agent_ids:
            raw_list = src.get(agent_id, [])
            if not isinstance(raw_list, list):
                continue
            ...
            for idx, item in enumerate(raw_list[: self.worker_queue_limit], start=1):
                ...
                try:
                    ...
                    target_station = None
                remaining_uses = self._clamp_int(item.get("remaining_uses"), 1, 3, 2 if message_type == "focus_window" else 1)
                cleaned.append(
                    {
    ...
    def _related_opportunity_ids(self, bottleneck_name: str, opportunities: list[dict[str, Any]]) -> list[str]:
        name = str(bottleneck_name or "").strip().lower()
        families: set[str] = set()
        if any(token in name for token in ("inspect", "inspection")):
            families.add("inspect_product")
        if any(token in name for token in ("broken", "repair", "breakdown", "reliability")):
            families.update({"repair_machine", "preventive_maintenance"})
        if any(token in name for token in ("unload", "output_buffer")):
            families.update({"unload_machine", "inter_station_transfer"})
        ...
        for item in opportunities:
            if not isinstance(item, dict):
                continue
            if str(item.get("task_family", "")).strip() not in families:
                continue
            opportunity_id = str(item.get("opportunity_id", "")).strip()
            if opportunity_id:
                rows.append(opportunity_id)
        return rows[:6]
    ...
    def _planning_packet(self, observation: dict[str, Any], strategy: StrategyState, norms: dict[str, Any]) -> dict[str, Any]:
        ...
        signals = self._worker_local_signals(observation)
        opportunities = self._current_opportunity_rows(observation)
        active_commitments = observation.get("active_commitments", {}) if isinstance(observation.get("active_commitments", {}), dict) else {}
        incident_context = self._compact_planner_incident_context(observation.get("incident_context", {}))
        ...
        detector_top_bottlenecks = [
            {
                **item,
                "related_opportunity_ids": self._related_opportunity_ids(item.get("name", ""), opportunities),
            }
            for item in detector_top_bottlenecks[: self.detector_max_top_bottlenecks]
            if isinstance(item, dict)
        ]
        ...
        return {
            ...
            "execution_state": {
                "days_remaining": int(time_view.get("days_remaining", 0) or 0),
                "current_plan_revision": int(getattr(self.current_job_plan, "plan_revision", 0) or 0),
            },
            "closure_signals": {
                "inspection_backlog": int(signals.get("inspection_backlog", 0) or 0),
                "station1_output_buffer": int(signals.get("station1_output_buffer", 0) or 0),
            ...
            "detector_hypothesis": {
                ...
                "top_bottlenecks": detector_top_bottlenecks,
            ...
            "guardrails": {
                "allowed_task_priority_keys": list(self.allowed_task_priority_keys),
                "allowed_agent_ids": list(self.agent_ids),
                "allowed_target_stations": [1, 2],
                "allowed_target_types": ["none", "station", "machine", "agent", "location"],
                "commitment_entry_contract": {
                    "opportunity_id": "str_from_opportunities",
                    "commitment_id": "str",
                    "alternate_workers": "list[str]",
                    "dependencies": "list[str]",
                    "expiry_min": "number|null",
                    "success_criteria": "list[str]",
                    "rationale": "str",
                },
                "mailbox_entry_contract": {
                    "message_type": "handover|coordination|dependency|watchout|assist_request",
                    "to_agent": "A1|A2|A3",
                    "body": "str",
                },
                "incident_strategy_contract": {
                    "mode": "delta_replan|maintain",
            ...
            rows.append(
                {
                    "day": int(item.get("day", 0) or 0),
                    "summary": self._truncate_prompt_text(item.get("summary", ""), max_len=140),
                    **self._compact_plan_focus(item.get("task_priority_weights", {}), item.get("personal_queues", {}), item.get("commitments", {})),
            ...
            daily_outcomes.append(
                {
                    ...
                    "unique_replan_blocker_count": int(item.get("unique_replan_blocker_count", 0) or 0),
                    "planner_escalation_count": int(item.get("planner_escalation_count", 0) or 0),
                }
            )
        return {
            "run_context": {
    ...
    def _plan_has_actionable_change(self, plan: JobPlan, fallback: JobPlan) -> bool:
        if isinstance(plan.commitments, dict):
            for agent_id in self.agent_ids:
                if list(plan.commitments.get(agent_id, [])) != list((fallback.commitments or {}).get(agent_id, [])):
                    return True
        if isinstance(plan.mailbox, dict):
            for agent_id in self.agent_ids:
                if list(plan.mailbox.get(agent_id, [])) != list((fallback.mailbox or {}).get(agent_id, [])):
                    return True
        if dict(getattr(plan, "incident_strategy", {}) or {}) != dict(getattr(fallback, "incident_strategy", {}) or {}):
            return True
        return False
    ...
    def _synthesize_actionable_plan_payload(self, observation: dict[str, Any], strategy: StrategyState, fallback: JobPlan) -> dict[str, Any]:
        ...
        if isinstance(fallback, JobPlan) and isinstance(fallback.commitments, dict):
            for agent_id in self.agent_ids:
                rows = fallback.commitments.get(agent_id, []) if isinstance(fallback.commitments.get(agent_id, []), list) else []
                for item in rows[:per_agent_limit]:
                    if not isinstance(item, dict):
                        continue
                    opportunity_id = str(item.get("opportunity_id", "")).strip()
                    opportunity = opportunity_index.get(opportunity_id)
                    ...
                    commitments[agent_id].append(
                        {
                            "commitment_id": str(item.get("commitment_id", f"FCMT-{agent_id}-{len(commitments[agent_id]) + 1}"))[:64],
                            "opportunity_id": opportunity_id,
                            "rationale": self._truncate_prompt_text(
                                item.get("rationale", opportunity.get("why_available", "")),
                                max_len=220,
                            ),
                            "success_criteria": [
                                self._truncate_prompt_text(value, max_len=120)
                                for value in self._as_str_list(item.get("success_criteria"), [])[:3]
                            ],
                        }
                    )
        ...
        for opportunity in opportunities:
            ...
            if not owners:
                owners = list(self.agent_ids)
            chosen_agent = ""
            for agent_id in owners:
                if len(commitments.get(agent_id, [])) < per_agent_limit:
            ...
            commitments[chosen_agent].append(
                {
                    "commitment_id": f"FCMT-{chosen_agent}-{len(commitments[chosen_agent]) + 1}",
                    "opportunity_id": opportunity_id,
                    "rationale": self._truncate_prompt_text(
                        f"Fallback selected {opportunity.get('task_family', '')} because it is currently feasible and directly tied to the active opportunity board.",
                        max_len=220,
                    ),
            ...
            reason_trace.append(
                {
                    "decision": "adjust",
                    "reason": self._truncate_prompt_text(
                        f"Fallback commitment assigned {opportunity.get('task_family', '')} via {opportunity_id} because no stronger executable plan was returned.",
                        max_len=220,
                    ),
                    "evidence": [
                        {
                        ...
                        {
                            "signal": "expected_output_impact",
                            "value": opportunity.get("expected_output_impact", 0.0),
                            "source": "opportunities",
                        },
                    ],
                    "affected_agents": [chosen_agent],
                    "task_families": [str(opportunity.get("task_family", "")).strip()],
                    "detector_relation": "follow",
                }
            )
            if sum(len(rows) for rows in commitments.values()) >= min(len(self.agent_ids), len(opportunities)):
                break
        ...
        if not any(commitments.values()) and opportunities:
            ...
            commitments[chosen_agent].append(
                {
                    "commitment_id": f"FCMT-{chosen_agent}-1",
                    "opportunity_id": str(top.get("opportunity_id", "")).strip(),
                    "rationale": self._truncate_prompt_text(top.get("why_available", ""), max_len=220),
        ...
        return {
            ...
            "incident_strategy": {
                "mode": "delta_replan",
                "focus_opportunity_ids": [row["opportunity_id"] for rows in commitments.values() for row in rows[:2] if isinstance(row, dict) and str(row.get("opportunity_id", "")).strip()],
    ...
    def _synthesize_plan_reasoning(self, candidate: dict[str, Any], observation: dict[str, Any], strategy: StrategyState, fallback: JobPlan) -> dict[str, Any]:
        ...
        plan = self._build_orchestrated_job_plan(merged, fallback, strategy, observation)
        signals = self._worker_local_signals(observation)
    ...
    def _ensure_actionable_manager_plan(self, llm_obj: dict[str, Any], observation: dict[str, Any], strategy: StrategyState, fallback: JobPlan) -> tuple[dict[str, Any], bool]:
        ...
        plan = self._build_orchestrated_job_plan(candidate, fallback, strategy, observation)
        inert = not self._plan_has_actionable_change(plan, fallback) and not self._is_explicit_stable_plan(candidate)
        if inert:
            return self._synthesize_actionable_plan_payload(observation, strategy, fallback), True
        return candidate, False

    def _build_orchestrated_job_plan(self, llm_obj: dict[str, Any], fallback: JobPlan, strategy: StrategyState, observation: dict[str, Any]) -> JobPlan:
        mailbox_src = llm_obj.get("mailbox_add", llm_obj.get("mailbox", {}))
        plan = JobPlan(
            task_priority_weights=dict(fallback.task_priority_weights),
            ...
            agent_priority_multipliers=dict(fallback.agent_priority_multipliers),
        )
        plan.plan_revision = max(1, int(getattr(fallback, "plan_revision", 0) or 0) + 1)
        plan.personal_queues = {aid: [] for aid in self.agent_ids}
        plan.commitments = self._sanitize_commitments(llm_obj.get("commitments"), observation, fallback)
        ...
        plan.incident_strategy = self._sanitize_incident_strategy(llm_obj.get("incident_strategy"))
        ...
        plan.ensure_commitments(self.agent_ids)
    ...
    def _sync_orchestration_plan_workspace(self, observation: dict[str, Any], strategy: StrategyState, job_plan: JobPlan) -> None:
        ...
        plan_payload = {
            ...
            "commitments": dict(job_plan.commitments),
            ...
            "incident_strategy": dict(getattr(job_plan, "incident_strategy", {}) or {}),
            "plan_revision": int(getattr(job_plan, "plan_revision", 0) or 0),
        }
        for aid in self.agent_ids:
            ...
            self._openclaw_write_json(workspace / "facts" / "current_commitments.json", job_plan.commitments.get(aid, []))
            ...
            self._openclaw_write_json(workspace / "plans" / f"day_{day:02d}_commitments.json", job_plan.commitments.get(aid, []))
    ...
    def _strategy_from_last_diagnosis(self) -> StrategyState:
        diagnosis_payload = self.last_diagnosis_payload if isinstance(self.last_diagnosis_payload, dict) else {}
        diagnosis_review = self.last_diagnosis_review if isinstance(self.last_diagnosis_review, dict) else {}
        top_bottlenecks = list(diagnosis_payload.get("top_bottlenecks", [])) if isinstance(diagnosis_payload.get("top_bottlenecks", []), list) else []
        summary = str(diagnosis_payload.get("summary", "")).strip() or self._synthesize_detector_summary(top_bottlenecks)
        return StrategyState(
            notes=self._flatten_diagnosis_to_notes(summary, {"top_bottlenecks": top_bottlenecks}),
            summary=summary,
            diagnosis={"top_bottlenecks": top_bottlenecks},
    ...
    def reflect(self, observation: dict[str, Any]) -> StrategyState:
        ...
        self.last_diagnosis_payload = {"summary": summary, "top_bottlenecks": list(diagnosis["top_bottlenecks"])}
    ...
    def propose_jobs(self, observation: dict[str, Any], strategy: StrategyState, norms: dict[str, Any]) -> JobPlan:
        ...
        fallback.commitments = {aid: [] for aid in self.agent_ids}
        ...
        system_prompt, prompt, required_keys = self._native_turn_prompts(
            ...
            required_fields={
                ...
                "incident_strategy": "dict[str, Any]",
            ...
            instructions=[
                ...
                "Use commitments as the authoritative execution output. Do not invent targets; reference opportunities.opportunity_id directly.",
                "Use maintain only when current evidence and relevant run-local memory show no materially stronger executable commitment than the active plan.",
                "Choose the intervention that most improves accepted finished-product completion over the remaining horizon.",
                "Use only task_family names from guardrails.allowed_task_priority_keys in commitments and reason_trace.",
                ...
                "If plan_mode=adjust, commitments must contain at least one worker-specific executable commitment when opportunities is non-empty.",
        ...
        try:
            ...
            llm_obj = self._synthesize_actionable_plan_payload(observation, strategy, fallback)
            llm_obj["manager_summary"] = self._truncate_prompt_text(
                f"Planner fallback applied after native-local planner failure: {exc}",
                max_len=300,
            )
            llm_obj.setdefault("incident_strategy", {})
            llm_obj["incident_strategy"]["watchouts"] = list(
                dict.fromkeys(
                    [*self._as_str_list((llm_obj.get("incident_strategy", {}) or {}).get("watchouts"), []), "planner_native_contract_failure"]
                )
            )[:4]
        llm_obj, inert_plan_detected = self._ensure_actionable_manager_plan(llm_obj, observation, strategy, fallback)
        plan = self._build_orchestrated_job_plan(llm_obj, fallback, strategy, observation)
    ...
    def urgent_discuss(self, event: dict[str, Any], local_state: dict[str, Any]) -> dict[str, Any]:
        if not self.incident_replan_enabled:
            return {}
        observation = local_state.get("observation", {}) if isinstance(local_state.get("observation", {}), dict) else {}
        if not observation:
            return {}
        norms = local_state.get("norms", {}) if isinstance(local_state.get("norms", {}), dict) else {}
        strategy = self._strategy_from_last_diagnosis()
        fallback = self.current_job_plan if isinstance(self.current_job_plan, JobPlan) else self._default_job_plan(norms, observation)
        payload = self._synthesize_actionable_plan_payload(observation, strategy, fallback)
        event_type = str(event.get("event_type", "")).strip()
        details = event.get("details", {}) if isinstance(event.get("details", {}), dict) else {}
        reason = str(details.get("reason", "")).strip()
        watchout = self._truncate_prompt_text(f"{event_type}:{reason}" if reason else event_type, max_len=96)
        if watchout:
            payload.setdefault("incident_strategy", {})
            payload["incident_strategy"]["watchouts"] = list(
                dict.fromkeys([*self._as_str_list((payload.get("incident_strategy", {}) or {}).get("watchouts"), []), watchout])
            )[:4]
        payload.setdefault("reason_trace", [])
        payload["reason_trace"] = [
            {
                "decision": "adjust",
                "reason": self._truncate_prompt_text(
                    f"Incident delta replan refreshed executable commitments after {event_type or 'replan_required'} changed local feasibility.",
                    max_len=220,
                ),
                "evidence": [
                    {
                        "signal": "event_type",
                        "value": event_type or "unknown_incident",
                        "source": "incident_context",
                    },
                    {
                        "signal": "active_blockers",
                        "value": len((observation.get('incident_context', {}) or {}).get('active_blockers', []))
                        if isinstance(observation.get("incident_context", {}), dict)
                        else 0,
                        "source": "incident_context",
                    },
                ],
                "affected_agents": list(self.agent_ids[:3]),
                "task_families": [
                    str(item.get("task_family", "")).strip()
                    for rows in payload.get("commitments", {}).values()
                    for item in rows[:1]
        ...
        plan = self._build_orchestrated_job_plan(payload, fallback, strategy, observation)
        self.current_job_plan = plan
        self.agent_priority_multipliers = self._clone_agent_priority_multipliers(plan.agent_priority_multipliers)
        return {
            "commitments": plan.commitments,
            "mailbox": plan.mailbox,
            "incident_strategy": dict(getattr(plan, "incident_strategy", {}) or {}),
            "reason_trace": list(plan.reason_trace),
