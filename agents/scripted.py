...
class ScriptedDecisionModule(DecisionModule):
    ...
    def __init__(self, cfg: dict[str, Any]) -> None:
        ...
        self.static_priority_policy = is_fixed_priority_mode(self.decision_mode)
    ...
    def propose_jobs(self, observation: dict[str, Any], strategy: StrategyState, norms: dict[str, Any]) -> JobPlan:
        ...
        min_pm = 1
        inspection_backlog_target = 8
        output_buffer_target = 4
        battery_reserve_min = 50.0
        quotas["pm_runs"] = min_pm * int(self._rule("propose_jobs.pm_runs_per_machine_multiplier", 1)) * 6
        if not self.static_priority_policy:
        ...
        if (not self.static_priority_policy) and min_battery < battery_reserve_min:
        ...
        if self.static_priority_policy:
            rationale_bits.append("static_priority_policy=true")
