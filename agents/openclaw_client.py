...
class OpenClawClient:
    ...
    def _ensure_wsl_ollama_proxy(self, *, listen_port: int, target_port: int, distro: str) -> bool:
        ...
        if self._http_get_ok(probe_url, timeout_sec=5.0):
            return True
        proxy_script = self._repo_root() / "agents" / "wsl_ollama_proxy.py"
    ...
    def _ensure_workspace_minimum(path: Path, agent_id: str) -> None:
        ...
        (path / "facts" / "request_history").mkdir(parents=True, exist_ok=True)
        (path / "facts" / "response_history").mkdir(parents=True, exist_ok=True)
        (path / "facts" / "memory_update_history").mkdir(parents=True, exist_ok=True)
        (path / "facts" / "worker_reports").mkdir(parents=True, exist_ok=True)
        (path / "facts" / "daily_review").mkdir(parents=True, exist_ok=True)
    ...
    def prepare_run_runtime(
        ...
        *,
        ...
        prune_unused_agents: bool = False,
    ) -> dict[str, Any]:
        resolved_output_root = Path(output_root).resolve(strict=False)
        run_date = "run"
        for parent in [resolved_output_root, *resolved_output_root.parents]:
            candidate = str(parent.name or "").strip()
            if re.fullmatch(r"\d{4}-\d{2}-\d{2}", candidate):
                run_date = candidate
                break
        output_hash = hashlib.sha1(str(resolved_output_root).encode("utf-8")).hexdigest()[:10]
        run_label = f"r_{output_hash}_p{os.getpid()}"
        safe_profile_name = re.sub(r"[^A-Za-z0-9_.-]+", "_", str(self.profile_name or "profile").strip()) or "profile"
        runtime_root = Path(tempfile.gettempdir()) / "ManSim" / "openclaw_runtime" / run_date / run_label / safe_profile_name
        ...
        explicit_runtime_agent_ids = [str(agent_id).strip() for agent_id in list(runtime_agent_ids or []) if str(agent_id).strip()]
        if explicit_runtime_agent_ids:
            base_agent_ids = explicit_runtime_agent_ids
        else:
        ...
        if not isinstance(agent_list, list):
            agent_list = []
            agents["list"] = agent_list
        allowed_agent_keys = {self._runtime_agent_key(agent_id) for agent_id in alias_map.keys()}
        if prune_unused_agents and allowed_agent_keys:
            pruned_agent_list: list[dict[str, Any]] = []
            for agent in agent_list:
                if not isinstance(agent, dict):
                    continue
                ...
                if normalized == "MAIN" or normalized in allowed_agent_keys:
                    pruned_agent_list.append(agent)
            agent_list = pruned_agent_list
            agents["list"] = agent_list
        ...
        return {
            ...
            "gateway_url": str(self.gateway_url),
    ...
    def native_agent_turn(
        ...
        *,
        ...
        if workspace is not None:
            ...
            request_preview = request_text.strip()
            template_preview = response_template_text.strip()
            missing_inputs: list[str] = []
            if not request_preview or request_preview == "{}":
                missing_inputs.append("current_request.json")
            if not template_preview or template_preview == "{}":
                missing_inputs.append("current_response_template.json")
            if missing_inputs:
                raise RuntimeError(
                    "OpenClaw native workspace input missing or empty: "
                    + ", ".join(missing_inputs)
                    + f" | agent_id={runtime_agent_id} | workspace_alias={workspace_alias} | workspace={workspace}"
                )
            active_turn = [
                "# Active Turn",
            ...
            for attempt in range(4):
                try:
                    ...
                    user_md_path.write_text(user_md_text, encoding="utf-8")
                    last_error = None
                    break
                except FileNotFoundError as exc:
                    last_error = exc
    ...
    def chat_completion(
        ...
        *,
        ...
        runtime_agent_id = str(agent_id or "").strip()
        gateway_model = f"openclaw/{runtime_agent_id.lower()}" if runtime_agent_id else "openclaw/default"
        request_payload = {
            "model": gateway_model,
            ...
            "metadata": {
                "agent_id": runtime_agent_id,
                "session_id": self.sanitize_session_id(session_key),
                "transport": "chat_compat",
                "backend_model": str(self.model or "").strip(),
            },
        }
        if self.inference_seed is not None:
            request_payload["seed"] = int(self.inference_seed)
        ...
        try:
            ...
            raise RuntimeError(f"OpenClaw chat_compat HTTP {exc.code}: {body}") from exc
        try:
            content = str(payload["choices"][0]["message"]["content"])
        except (KeyError, IndexError, TypeError) as exc:
            raise RuntimeError(f"OpenClaw chat_compat response format error: {exc}") from exc
