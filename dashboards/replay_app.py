...
def _discover_manifest(events_path: Path) -> Path | None:
    candidates = [
        events_path.parent / "dashboard_manifest.json",
        events_path.parent.parent / "dashboard_manifest.json",
    ]
...
def main() -> None:
    ...
    query_events_path = _query_param_text("events_path")
    query_manifest_path = _query_param_text("manifest_path")
    query_series_root = _query_param_text("series_root")
    query_run_id = _query_param_text("run")

    manifest_path: Path | None = None
    if query_manifest_path:
        candidate = Path(query_manifest_path)
        if candidate.exists():
            manifest_path = candidate
    elif query_series_root:
        candidate = Path(query_series_root) / "dashboard_manifest.json"
        if candidate.exists():
            manifest_path = candidate
    elif query_events_path:
        manifest_path = _discover_manifest(Path(query_events_path))
    elif latest is not None:
        manifest_path = _discover_manifest(latest)

    manifest = _load_manifest(str(manifest_path)) if manifest_path is not None else {}
    runs = manifest.get("runs", []) if isinstance(manifest.get("runs", []), list) else []
    selected_run_id = query_run_id or str(manifest.get("current_run", "")).strip()
    selected_run: dict[str, Any] | None = None
    if runs:
        if not selected_run_id:
            selected_run_id = str((runs[-1] if isinstance(runs[-1], dict) else {}).get("id", "")).strip()
        for run in runs:
            if isinstance(run, dict) and str(run.get("id", "")).strip() == selected_run_id:
                selected_run = run
                break
        if selected_run is None and runs and isinstance(runs[-1], dict):
            selected_run = runs[-1]
            selected_run_id = str(selected_run.get("id", "")).strip()

    sidebar = st.sidebar
    sidebar.header("Replay Controls")
    if selected_run is not None:
        run_ids = [str(run.get("id", "")).strip() for run in runs if isinstance(run, dict)]
        labels = {str(run.get("id", "")).strip(): str(run.get("label", run.get("id", ""))).strip() for run in runs if isinstance(run, dict)}
        selected_run_id = sidebar.selectbox(
            "Run",
            options=run_ids,
            index=max(0, run_ids.index(selected_run_id)) if selected_run_id in run_ids else max(0, len(run_ids) - 1),
            format_func=lambda raw: labels.get(raw, raw),
        )
        for run in runs:
            if isinstance(run, dict) and str(run.get("id", "")).strip() == selected_run_id:
                selected_run = run
        ...
        try:
            if manifest_path is not None:
                st.query_params["manifest_path"] = str(manifest_path.resolve().as_posix())
    ...
    if isinstance(selected_run, dict):
        ...
        selected_events = str(artifacts.get("events.jsonl", "")).strip()
        if selected_events:
            default_events_path = selected_events

    events_path_input = sidebar.text_input("events.jsonl path", value=default_events_path)
    ...
    if selected_run is not None:
        kpi = selected_run.get("kpi", {}) if isinstance(selected_run.get("kpi", {}), dict) else {}
        sidebar.markdown("---")
        sidebar.caption(f"Current run: {selected_run_id}")
