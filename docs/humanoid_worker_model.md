...
## WAITING vs BLOCKED 기준

ManSim은 `WAITING`과 `BLOCKED`를 다음 기준으로 구분합니다.

- `WAITING`: 현재 task의 전제는 아직 유효하지만, resource/input/traffic/battery support 같은 조건이 잠깐 풀리기를 기다리는 상태입니다. 같은 task를 이어서 수행할 수 있다는 가정이 남아 있습니다.
