...
- `manufacturing_sim/simulation/scenarios/manufacturing/grid_map.py`
  - Tile map, pathfinding, object footprints, worker tile occupancy.
...
정상적으로 실행 중인 모든 primitive는 `availability=EXECUTING`입니다. Mobility와 Manipulation은 HumanoidSim primitive별 state relation을 따릅니다. 단, incident recovery protocol 안에서 실행되는 task/primitive는 정상 작업이 아니라 blocked 상태의 복구 절차이므로 availability를 `BLOCKED`로 유지하고 현재 step만 Task 또는 Primitive context에 `CODE (RECOVERY)`로 기록합니다.
...
Item은 material, intermediate, product, battery 등으로 구분합니다. 주요 state:
...
Item state는 Humanoid state와 별개입니다. Worker가 item을 들면 worker의 `manipulation`은 보통 `HOLDING`이 되고, item state는 `CARRIED_BY_WORKER`가 됩니다. Item drop incident가 발생하면 item은 현재 tile에 `DROPPED` 상태로 남고, recovery protocol이 다시 집는 과정을 수행합니다.
...
- Fail item은 inspection scrap queue로 들어가며 이때 `scrap_count`가 증가합니다.
...
- 기본 capacity는 `scenario.warehouse.material_shelf.capacity`입니다.
- 시작 시 `initial_fill`만큼 채우고 `restock_policy: day_boundary`이면 매일 시작 때 빈 slot만 capacity까지 보충합니다.
...
- Shelf와 shelf 뒤 wall은 tile map의 blocking footprint/wall로 기록되어 pathfinding과 3D Replay에 반영됩니다.
...
Repair에는 여러 worker가 같은 machine에 합류할 수 있습니다. 동시 repair worker 수는 `scenario.machine_failure.max_repair_agents`가 제한합니다. Preventive maintenance는 idle machine에 대해 수행하며 breakdown probability를 낮추는 효과를 가집니다.
...
Product transport session이 active이고 carrier가 1명이며 잔여 경로가 충분하면 다른 available worker가 `HANDOVER_ITEM` 후보를 받을 수 있습니다. Helper가 합류하면 다음 tile segment부터 product 이동 multiplier가 carrier 수로 나뉩니다.
...
Core는 event-sourced replay를 위해 주요 상태 변화를 기록합니다. Worker 관련 event details에는 `humanoid_state` snapshot 원본이 들어갑니다.
...
Worker panel은 `humanoid_state` 원본에서 state 축, task context, primitive, cargo item id, traffic reason을 읽습니다. Task가 종료되어 `task_context`가 비면 stale task label을 표시하지 않습니다.

Nested task가 끝나도 parent task가 계속 실행 중이면 replay export는 parent `task_window`를 유지합니다. 예를 들어 `REPAIR_MACHINE -> INSPECT_MACHINE -> EXECUTE_MAINTENANCE_ACTION` 흐름에서는 child task가 먼저 끝나도 worker task gauge는 parent task 기준으로 계속 진행합니다.

3D Replay는 entity로 없는 static object도 layout footprint에서 읽을 수 있습니다. `inspection_table`은 `layout.grid.object_footprints`에 있으면 검사 테이블 block model로 표시하고, 오래된 log처럼 footprint가 없으면 Inspection region 안에 fallback 검사대를 표시합니다.
...
- shelf empty/restock metrics
- scrap queue/disposal metrics
...
LLM Wiki, Graphify graph, manager replay는 core 바깥의 orchestration/dashboard layer에서 생성합니다.
